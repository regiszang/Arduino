...
//fim info sd card
   ...
   */
//informaoes do Sensor de Temperatura
...
//Fim Dados Globais SD Card
 ...
 // temperature=Clock.getTemperature();
  ...
  lcd.print(' ');
  float LcdTemp=sensor.getTempCByIndex(0);
    {lcd.print(LcdTemp);}
    lcd.print('c');
  }
  ...
  void loop() {
   ...
   //inicia Parametro Min Temperatura
    ...
    float leitura=sensor.getTempCByIndex(0);
      ...
      //Teste Qualidade Temperatura Truta
      if (leitura < vtemp_min) 
          {
        //Acende Azul
        varTestaTemp = String("Menor de 15");
          }
    else if (leitura > vtemp_max)
          {
        //Acende o Vermelho
        varTestaTemp = String("Maior de 22");
          }

    else
        {
      //Acende o verde
        varTestaTemp = String("entre 15 e 22");
        }
  ...
  ///Serial.print(ReadDS3231());
    ////inicio loop SD Card
                            ...
                            if (sensorFlag ==1)    //if there is a sensor reading...
                          {
                            ...
                            dataString = String(varDataCerta) +  ","  + String(varHoraCerta) + " T1-" + String(leitura) + String(",") + " Sts - - > " + String(varTestaTemp) + " V2" + String(sensorValue); //concatenate (add together) a string consisting of the time and the sensor reading at that time
